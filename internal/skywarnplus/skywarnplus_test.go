...
func TestGetStatusSuccess(t *testing.T) {
	fakePython3(t, true, `{"enable":true,"sayalert":false,"sayallclear":true,"tailmessage":false,"countycodes":["ARC125"],"nodes":["2000"]}`, "")
	status, err := GetStatus(context.Background(), "/unused/dir")
	if err != nil {
		t.Fatalf("GetStatus() error = %v", err)
	}
	want := Status{Enable: true, SayAlert: false, SayAllClear: true, Tailmessage: false, CountyCodes: []string{"ARC125"}, Nodes: []string{"2000"}}
	if status.Enable != want.Enable || status.SayAlert != want.SayAlert || status.SayAllClear != want.SayAllClear || status.Tailmessage != want.Tailmessage {
